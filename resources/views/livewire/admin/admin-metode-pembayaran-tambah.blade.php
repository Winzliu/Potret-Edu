<div class="pt-8 pe-8 xl:pe-0">
    {{-- START --}}
    <a href="/admin/metode-pembayaran" wire:navigate class="flex items-center gap-2 font-bold">
        <svg class="
    ...
    <p class="font-bold text-3xl text-center">Tambah Metode Pembayaran</p>
    {{-- form --}}
    <div
        ...
        <div class="flex gap-x-5 gap-y-2 pb-8 px-10 justify-center items-start pt-5 ">
            ...
            <label class="form-control w-full max-w-md">
                <div class="label">
                    <span class="label-text font-semibold text-white">Nama Metode Pembayaran</span>
                </div>
                <input type="text" placeholder="Nama Metode Pembayaran" wire:model="method"
            ...
            <label class="form-control w-full max-w-md">
                <div class="label">
                    <span class="label-text font-semibold text-white">Persentase Pajak (%)</span>
                </div>
                <input type="number" placeholder="Persentase Metode Pembayaran (Nilai antara 1-100)" wire:model="taxes"
                    class="input input-bordered w-full max-w-md cursor-pointer text-sm" />
                @error('taxes')
                <i class="text-red-500 ms-4 mt-0 mb-0 font-medium">
                    *{{ $message }}
                ...
                @enderror
                {{-- akhir persentase metode pembayaran --}}
            </label>
        </div>
    ...
    {{-- MODAL HAPUS  --}}
        ...
        <div class="fixed top-0 left-0 w-full h-full bg-slate-500 bg-opacity-30 flex justify-center items-center">
            <div class="bg-background px-12 py-10 rounded-xl shadow-lg text-center border-4 border-tertiaryColor font-semibold">
                <p>Apakah Anda yakin ingin menambah Metode Pembayaran?</p>
                <div class="mt-4 flex flex-row justify-center gap-8 font-medium">
                    ...
                    <button wire:click="buatMetodePembayaran" class="bg-greenConfirm hover:bg-green-500 text-white px-10 py-2 rounded-md mr-2">Ya</button>
                </div>
            </div>
        </div>
        @endif
    ...
    {{-- loading --}}
    <dialog wire:loading wire:target="buatMetodePembayaran" wire:loading.attr="open"
